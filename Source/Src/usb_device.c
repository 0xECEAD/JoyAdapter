...
__ALIGN_BEGIN static uint8_t HID_CfgDesc[HID_CONFIG_DESC_SIZ] __ALIGN_END =
{
   0x09,
...
static uint8_t  *HID_GetCfgDesc(uint16_t *length)
{
   *length = sizeof(HID_CfgDesc);
   return HID_CfgDesc;
}

void SetupCfgDesc()
{
   USBD_CUSTOM_HID.GetFSConfigDescriptor = USBD_CUSTOM_HID.GetHSConfigDescriptor = USBD_CUSTOM_HID.GetOtherSpeedConfigDescriptor = 
      HID_GetCfgDesc;
}
/* USER CODE END 1 */
